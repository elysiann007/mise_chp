...
export default function NotFound() {
  return (
    <PageWrapper>
      <main className="min-h-screen bg-stone-950 flex items-center justify-center px-5 pt-20">
        <motion.div
          variants={stagger}
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          <motion.h1
            variants={fadeUp}
            className="font-display text-white -mt-4 mb-4"
            style={{ fontSize: 'clamp(1.4rem, 4vw, 2.2rem)', letterSpacing: '0.12em' }}
          >
            PAGE NOT FOUND
          </motion.h1>

          <motion.p variants={fadeUp} className="text-zinc-500 text-sm mb-10 max-w-xs mx-auto leading-relaxed">
          ...
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              to="/menu"
              className="px-8 py-3.5 border border-zinc-700 text-zinc-400 font-semibold text-xs tracking-[0.2em] uppercase rounded-full hover:border-amber-400/50 hover:text-white transition-all duration-300"
            >
              View Menu
            </Link>
          </motion.div>
