...
import { ALCOHOLIC_DRINKS } from '../../../constants/menu'

export default function AlcoholicMenu() {
    ...
    <PageWrapper>
      <main className="min-h-screen bg-stone-50 dark:bg-stone-950 pt-20">
        <div className="relative py-20 px-5 border-b border-stone-200 dark:border-zinc-800/60 overflow-hidden">
          <div className="absolute bottom-0 right-0 w-80 h-80 bg-amber-600/5 rounded-full blur-[100px] pointer-events-none" />
          <motion.div variants={stagger} initial="hidden" animate="visible" className="max-w-3xl mx-auto relative z-10">
            ...
            <motion.h1 variants={fadeUp} className="font-display text-stone-900 dark:text-white" style={{ fontSize: 'clamp(3rem, 10vw, 7rem)', letterSpacing: '0.04em' }}>
              {t('hub.al_title')}
            </motion.h1>
        ...
        <div className="max-w-3xl mx-auto px-5 py-16 space-y-20">
          {ALCOHOLIC_DRINKS.map((section, si) => (
            <motion.section key={section.id} variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-60px' }}>
              <motion.div variants={slideLeft} className="flex items-center gap-5 mb-10">
                <h2 className="font-display text-amber-600 dark:text-amber-400 flex-shrink-0" style={{ fontSize: 'clamp(2rem, 6vw, 3.5rem)', letterSpacing: '0.1em' }}>
          ...
          <motion.div variants={fadeIn} initial="hidden" whileInView="visible" viewport={{ once: true }} className="border-t border-stone-200 dark:border-zinc-800/60 pt-8 text-center">
            <p className="text-stone-300 dark:text-zinc-700 text-xs leading-relaxed">{t('drinks.disclaimer')}</p>
          </motion.div>
        </div>
      </main>
    </PageWrapper>
