...
**Auto-generated — do not hand-edit.** `menu.ts` and the `food.items` / `drinks.items` / category-label keys in every `src/i18n/locales/*.json` file are regenerated by `mise-frontend/scripts/sync-menu.ts` from a Google Sheet (owner-maintained) on a ~20-minute schedule via `.github/workflows/sync-menu.yml`. See `docs/menu-sync-setup.md` for the Sheet template and setup steps. Manual edits to these files will be overwritten by the next sync run.

Structure:
```ts
FOOD_MENU = [{ id, category, items: [{ key, name, desc, price, calories }] }]
NON_ALCOHOLIC_DRINKS = [{ id, category, items: [{ key, name, desc, price, calories }] }]
ALCOHOLIC_DRINKS = [{ id, category, items: [{ key, name, desc, price, calories }] }]
```
...
New category ids (from the Sheet) are added automatically, with a Gemini-translated heading in all 10 locales.
...
├── scripts/
│   └── sync-menu.ts      # Google Sheet → menu.ts + locale files (see docs/menu-sync-setup.md)
├── public/
│   ├── robots.txt
│   └── sitemap.xml
└── src/
...
- **CMS / Owner Admin Panel:** ~~Owner currently edits `menu.ts` or GitHub directly.~~ Solved via the Google Sheet → GitHub Action sync (see `docs/menu-sync-setup.md`). Owner edits a Sheet in Turkish; prices/items/new categories sync to the live site automatically, translated into all 10 languages via Gemini. Possible future upgrade: a proper CMS (e.g. Sanity) if the Sheet-based flow proves limiting.
...
- `src/constants/menu.ts` and the `food`/`drinks` `items`/category-label keys in `src/i18n/locales/*.json` — auto-generated by the menu sync (see above), don't hand-edit.
